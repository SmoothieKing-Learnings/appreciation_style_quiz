...
The **SmoothieKing Learnings unified iframe contract**, shared across every experience in the `sk-learning` repo. The bridge lives at [`src/utils/iframeBridge.js`](./src/utils/iframeBridge.js); the only value that differs per project is the message namespace. For this project the namespace is **`appreciationQuiz`** and the deployed URL is:

> `https://smoothieking-learnings.github.io/appreciation_style_quiz/`
...
```html
<iframe
  src="https://smoothieking-learnings.github.io/appreciation_style_quiz/?embed=1"
  width="100%"
  height="700"
  style="border:0; display:block; width:100%;"
  scrolling="auto"
  title="Appreciation Style Quiz"
...
```html
<iframe
  src="https://smoothieking-learnings.github.io/appreciation_style_quiz/?embed=1&parentOrigin=https%3A%2F%2Frise.articulate.com"
  width="100%" height="700"
  style="border:0; display:block; width:100%;" scrolling="auto"
  title="Appreciation Style Quiz" allow="autoplay"></iframe>
```
...
<div id="app-wrap">
  <iframe id="app-iframe"
    src="https://smoothieking-learnings.github.io/appreciation_style_quiz/?embed=1&parentOrigin=https%3A%2F%2Frise.articulate.com"
    title="Appreciation Style Quiz" allow="autoplay" allowfullscreen></iframe>
  <div id="app-overlay" role="button" aria-label="Click to start the Appreciation Style Quiz"></div>
</div>
...
<script>
  (function () {
    ...
    window.addEventListener('message', function (e) {
      var d = e.data;
      if (d && typeof d === 'object' && d.type === 'appreciationQuiz:results') {
...
For the cleanest, most native-feeling embed: replicate the quiz's welcome card inside the Code Block, drop the iframe behind it with `pointer-events: none`, and post `appreciationQuiz:start` when the learner clicks **Let's Blend!**. Pass `?autostart=1` so the iframe skips its own welcome the moment it becomes interactive.

The full markup for this **Pattern C** is in section 5 of [`../leadership_style_quiz/RISE360_INTEGRATION_GUIDE.md`](../leadership_style_quiz/RISE360_INTEGRATION_GUIDE.md) — too long to inline here, but a copy-paste affair. Substitute `myApp` → `appreciationQuiz` in the markup.

### 2.7 Code Block — scroll-eavesdrop (no engagement gesture)

The cleanest interaction model: the iframe is fully interactive immediately (no overlay, no `pointer-events: none`, no "Click to start" pill), and scroll passes through to the Rise lesson because the parent listens for the bridge's `appreciationQuiz:wheel` postMessage and calls `window.scrollBy()`.
...
```html
<style>
  #appreciation-wrap { position: relative; width: 100%; }
  #appreciation-wrap iframe {
    display: block; width: 100%; height: 700px; border: 0;
  }
  @media (max-width: 520px) {
    #appreciation-wrap iframe { height: 90vh; max-height: 720px; min-height: 600px; }
  }
</style>

<div id="appreciation-wrap">
  <iframe
    src="https://smoothieking-learnings.github.io/appreciation_style_quiz/?embed=1&parentOrigin=https%3A%2F%2Frise.articulate.com"
    title="Appreciation Style Quiz" allow="autoplay" allowfullscreen></iframe>
</div>
...
<script>
  (function () {
    window.addEventListener('message', function (e) {
      ...
      if (d.type === 'appreciationQuiz:wheel' && typeof d.deltaY === 'number') {
...
The bridge namespaces every outbound event with `appreciationQuiz:`. The `complete` signal is intentionally **not** namespaced — Rise 360's completion field listens for that exact bare message.

### Outbound — app → host

| Event | Payload | Fires when |
| --- | --- | --- |
| `appreciationQuiz:ready` | — | App has mounted |
| `appreciationQuiz:start` | — | User started the quiz |
| `appreciationQuiz:results` | `{ topStyles, allScores }` | Results screen rendered |
| `appreciationQuiz:restart` | — | User restarted from results |
| `appreciationQuiz:resize` | `{ width, height, desiredHeight }` | Mount + resize + orientation change |
| `appreciationQuiz:wheel` | `{ deltaY }` | rAF-throttled wheel forwarding |
...
| Event | Effect |
| --- | --- |
| `appreciationQuiz:start` | Start the quiz immediately, skipping welcome |
| `appreciationQuiz:restart` | Return to the welcome screen |
...
window.addEventListener('message', (e) => {
  ...
  if (!String(data.type).startsWith('appreciationQuiz:')) return
  switch (data.type) {
    case 'appreciationQuiz:ready':    /* iframe mounted */                  break
    case 'appreciationQuiz:start':    /* user started the quiz */            break
    case 'appreciationQuiz:results':  /* { topStyles, allScores } */         break
    case 'appreciationQuiz:restart':  /* user restarted from results */      break
    case 'appreciationQuiz:resize':   /* { width, height, desiredHeight } */ break
    case 'appreciationQuiz:wheel':    /* { deltaY } */                       break
  }
})
```
...
iframe.contentWindow.postMessage({ type: 'appreciationQuiz:start' },   '*')
iframe.contentWindow.postMessage({ type: 'appreciationQuiz:restart' }, '*')
```
...
| `Esc` | (Welcome screen) clear the name field when focused |
...
The bridge emits `appreciationQuiz:wheel { deltaY }` on every wheel event inside the iframe as a best-effort signal for non-Rise hosts that *can* briefly toggle `pointer-events: none` based on it.
...
1. Open the published GitHub Pages URL directly in a browser:
   `https://smoothieking-learnings.github.io/appreciation_style_quiz/?embed=1`
...
| `src/data/stylesData.js` | Style definitions (name, color, description) |
...
- **Tall iframe (~720px)** → mobile content fits without internal scroll, desktop has visible blank space below content. Hits the Articulate Code Block 750 ceiling.
...
*Last unified: 2026-06-05. Sibling docs: [communication_style_quiz](../communication_style_quiz/IFRAME_EMBED.md) · [leadership_style_quiz](../leadership_style_quiz/IFRAME_EMBED.md) · [leadership_thermostat_game](../leadership_thermostat_game/IFRAME_EMBED.md).*
